...
`/handoff --output <file>` writes the same closing summary to a Markdown file. The web pilot exposes the same closing summary through its API so the local panel can fetch a handoff without entering the shell, and the right rail shows a refreshable handoff panel.
