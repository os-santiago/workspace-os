...
`/cycle` or `workspace cycle` orchestrates longer implementation runs with explicit health, stability, security, and quality checkpoints between iterations. `cycle run` executes a multi-iteration cycle against the active cycle or creates one when given `--label` and `--objective`, `cycle checkpoint` captures the current gate state, `cycle report` summarizes the active or selected cycle, and `cycle status` shows the latest checkpoint summary.
