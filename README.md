...
`/handoff --output <file>` writes the same closing summary to a Markdown file. `/batch handoff` and `/process handoff` export a scoped closing summary for the active batch or process, and both accept `--output <file>` plus `--compact` for a shorter report. `batch stop` and `process stop` also write a default `handoff.md` beside the local memory store so each completed window leaves a closing artifact. The web pilot exposes the same closing summary through its API so the local panel can fetch a handoff without entering the shell, and the right rail now includes refresh and download actions for the handoff panel.
