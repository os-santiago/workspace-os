...
class WorkspaceMemoryStore:
    ...
    def ensure_schema(self) -> None:
        ...
        with self._connection() as conn:
            ...
            conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS pr_traceability (
                    id INTEGER PRIMARY KEY AUTOINCREMENT,
                    ...
                    pr_state TEXT NOT NULL CHECK(pr_state IN ('open', 'closed', 'merged', 'draft')),
                    issue_number INTEGER,
                    work_item_number TEXT,
                    cycle_id INTEGER,
                    closing_keywords_verified INTEGER NOT NULL DEFAULT 0,
                    required_checks_status TEXT,
                    created_at TEXT NOT NULL,
                    updated_at TEXT NOT NULL,
                    merged_at TEXT,
                    closed_at TEXT,
                ...
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_issue_number ON pr_traceability(issue_number);
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_work_item ON pr_traceability(work_item_number);
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_cycle_id ON pr_traceability(cycle_id);
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_branch_name ON pr_traceability(branch_name);
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_pr_state ON pr_traceability(pr_state);
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_head_commit ON pr_traceability(head_commit_sha);
                CREATE INDEX IF NOT EXISTS idx_pr_traceability_merge_commit ON pr_traceability(merge_commit_sha);
                """
            )
    ...
    def record_pr_creation(
        ...
        work_item_number: str | None = None,
        cycle_id: int | None = None,
        closing_keywords_verified: bool = False,
        required_checks_status: str | None = None,
        created_at: str | None = None,
    ) -> int:
        """Record a new PR creation in the traceability table."""
        now = created_at or _utc_now()
        with self._connection() as conn:
            cursor = conn.execute(
                """
                INSERT INTO pr_traceability (
                    pr_number, pr_url, branch_name, head_commit_sha, merge_commit_sha,
                    pr_state, issue_number, work_item_number, cycle_id,
                    closing_keywords_verified, required_checks_status,
                ...
                ON CONFLICT(pr_number) DO UPDATE SET
                    ...
                    work_item_number = excluded.work_item_number,
                    cycle_id = excluded.cycle_id,
                    closing_keywords_verified = excluded.closing_keywords_verified,
                    required_checks_status = excluded.required_checks_status,
                    updated_at = excluded.updated_at
                """,
                (
                    pr_number,
                    pr_url.strip(),
                    branch_name.strip(),
                    head_commit_sha.strip(),
                    pr_state.strip(),
                    issue_number,
                    work_item_number.strip() if work_item_number else None,
                    cycle_id if cycle_id is not None else self.active_cycle_id(conn),
                    1 if closing_keywords_verified else 0,
                    required_checks_status.strip() if required_checks_status else None,
                    now,
                    now,
                ),
            )
    ...
    def update_pr_merge(
        ...
        """Update PR record when it gets merged."""
        now = _utc_now()
        with self._connection() as conn:
            conn.execute(
                """
                UPDATE pr_traceability
    ...
    def update_pr_state(
        ...
        """Update PR state (open, closed, draft, merged)."""
        now = _utc_now()
        with self._connection() as conn:
            if pr_state == "closed" and closed_at is None:
                closed_at = now
            conn.execute(
                """
                UPDATE pr_traceability
    ...
    def update_pr_checks(
        self,
        pr_number: int,
        required_checks_status: str,
        closing_keywords_verified: bool | None = None,
    ) -> None:
        """Update PR required checks status and keyword verification."""
        now = _utc_now()
        with self._connection() as conn:
            if closing_keywords_verified is not None:
                conn.execute(
                    """
                    UPDATE pr_traceability
                    SET required_checks_status = ?,
                        closing_keywords_verified = ?,
                        updated_at = ?
                    WHERE pr_number = ?
                    """,
                    (
                        required_checks_status.strip(),
                        1 if closing_keywords_verified else 0,
                        now,
                        pr_number,
                    ),
    ...
    def get_pr_by_number(self, pr_number: int) -> dict[str, object] | None:
        """Get PR record by PR number."""
        with self._connection() as conn:
            row = conn.execute(
                """
                SELECT id, pr_number, pr_url, branch_name, head_commit_sha, merge_commit_sha,
                       pr_state, issue_number, work_item_number, cycle_id,
                       closing_keywords_verified, required_checks_status,
                       created_at, updated_at, merged_at, closed_at
                FROM pr_traceability
                WHERE pr_number = ?
                """,
                (pr_number,),
            ).fetchone()
            ...
            return {
                ...
                "merge_commit_sha": str(row["merge_commit_sha"]) if row["merge_commit_sha"] else None,
                "pr_state": str(row["pr_state"]),
                "issue_number": int(row["issue_number"]) if row["issue_number"] is not None else None,
                "work_item_number": str(row["work_item_number"]) if row["work_item_number"] else None,
                "cycle_id": int(row["cycle_id"]) if row["cycle_id"] is not None else None,
                "closing_keywords_verified": bool(row["closing_keywords_verified"]),
                "required_checks_status": str(row["required_checks_status"]) if row["required_checks_status"] else None,
                "created_at": str(row["created_at"]),
    ...
    def get_pr_by_issue(self, issue_number: int) -> list[dict[str, object]]:
        """Get all PRs associated with a specific issue number."""
        with self._connection() as conn:
            rows = conn.execute(
                """
                SELECT id, pr_number, pr_url, branch_name, head_commit_sha, merge_commit_sha,
    ...
    def get_issue_by_pr(self, pr_number: int) -> int | None:
        """Get the issue number associated with a PR."""
        pr = self.get_pr_by_number(pr_number)
        if pr is None:
            return None
        return pr.get("issue_number")

    def get_prs_by_cycle(self, cycle_id: int) -> list[dict[str, object]]:
        """Get all PRs associated with a specific cycle."""
        with self._connection() as conn:
            rows = conn.execute(
                """
                SELECT id, pr_number, pr_url, branch_name, head_commit_sha, merge_commit_sha,
    ...
    def get_prs_by_work_item(self, work_item_number: str) -> list[dict[str, object]]:
        """Get all PRs associated with a specific work item number."""
        with self._connection() as conn:
            rows = conn.execute(
                """
                SELECT id, pr_number, pr_url, branch_name, head_commit_sha, merge_commit_sha,
                       ...
                       created_at, updated_at, merged_at, closed_at
                FROM pr_traceability
                WHERE work_item_number = ?
                ORDER BY created_at DESC
                """,
                (work_item_number.strip(),),
            )
            return [
                {
                    "id": int(row["id"]),
    ...
    def get_pr_by_branch(self, branch_name: str) -> dict[str, object] | None:
        """Get the most recent PR for a specific branch."""
        with self._connection() as conn:
            row = conn.execute(
                """
                SELECT id, pr_number, pr_url, branch_name, head_commit_sha, merge_commit_sha,
    ...
    def pr_traceability_stats(self) -> dict[str, int]:
        """Get statistics for PR traceability."""
        with self._connection() as conn:
            row = conn.execute(
                """
                SELECT
                    COUNT(*) AS total,
                    SUM(CASE WHEN pr_state = 'open' THEN 1 ELSE 0 END) AS open_count,
                    SUM(CASE WHEN pr_state = 'merged' THEN 1 ELSE 0 END) AS merged_count,
                    SUM(CASE WHEN pr_state = 'closed' THEN 1 ELSE 0 END) AS closed_count,
                    SUM(CASE WHEN pr_state = 'draft' THEN 1 ELSE 0 END) AS draft_count,
                    SUM(CASE WHEN closing_keywords_verified = 1 THEN 1 ELSE 0 END) AS keywords_verified_count,
                    SUM(CASE WHEN issue_number IS NOT NULL THEN 1 ELSE 0 END) AS with_issue_count,
                    SUM(CASE WHEN work_item_number IS NOT NULL THEN 1 ELSE 0 END) AS with_work_item_count,
                    SUM(CASE WHEN cycle_id IS NOT NULL THEN 1 ELSE 0 END) AS with_cycle_count
                FROM pr_traceability
                """
            ).fetchone()
            return {
                ...
                "merged_count": int(row["merged_count"] or 0) if row else 0,
                "closed_count": int(row["closed_count"] or 0) if row else 0,
                "draft_count": int(row["draft_count"] or 0) if row else 0,
                "keywords_verified_count": int(row["keywords_verified_count"] or 0) if row else 0,
                "with_issue_count": int(row["with_issue_count"] or 0) if row else 0,
                "with_work_item_count": int(row["with_work_item_count"] or 0) if row else 0,
                "with_cycle_count": int(row["with_cycle_count"] or 0) if row else 0,
            }
        ...
        with self._connection() as conn:
            rows = conn.execute(
                ...
                SELECT kind, title, body, created_at FROM (
                    ...
                    UNION ALL
                    SELECT 'pr' AS kind, 'PR #' || pr_number AS title, pr_url || ' [' || pr_state || ']' AS body, created_at
                    FROM pr_traceability
                    WHERE CAST(pr_number AS TEXT) LIKE :needle OR pr_url LIKE :needle OR branch_name LIKE :needle OR pr_state LIKE :needle
                )
                ORDER BY created_at DESC
                LIMIT :limit
                """,
